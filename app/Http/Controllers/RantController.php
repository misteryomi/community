...
class RantController extends Controller
{
    ...
    function __construct(Rant $rant, Mood $mood) {
        $this->rant = $rant;
        $this->mood = $mood;
    ...
    public function new($mood = null) {
        ...
        $mood = $this->mood->where('slug', $mood)->first();

        if(!$mood) {
            $moods = $this->mood->ordered(); //->where('is_parent', true)->
        } else {
            $moods = $this->mood->where('parent_id', $mood->id)->ordered();
        }

        return view('rants.new', compact('moods', 'mood'));
    }
    ...
    public function store(Request $request) {
        ...
        // //Fetch images in this rant
        // $images = $this->fetchImages($request->details);

        // if(count($images) > 0) {
        //     $this->updateMedia($rant, $images);
        // }
