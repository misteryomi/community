...
<a href="{{ $route }}" class="blog-post mb-sm-3">
  ...
  @endif
    ...
    <div class="blog-post-content p-sm-1">
        ...
        <h3>{!! request()->has('q') ? $post->highlightSearchQuery($post->title, request()->q) : $post->title !!} @if($post->meta && $post->meta->comment_answer_id)<small class="button small soft-warning">Answered</small>@endif</h3>
        @if(!$agent->isMobile())
